...
/**
 * GlowCursor — React Bits trailing-glow pointer.
 *
 * Ported from the Claude Design handoff (`GlowCursor.jsx`). The shader is
 * verbatim; the loader changed from an esm.sh side-load to a real `ogl`
 * import, and the component is fullscreen-only, which is the only mode the
 * Nocturne design uses.
 *
...
const GlowCursor = ({
    ...
    // Live config, read inside the rAF loop so prop changes take effect
    ...
    useEffect(() => {
        ...
        const program = new Program(gl, {
            ...
            uniforms: {
                ...
                uPointCount: { value: initial.trailLength },
                uColor: { value: hexToRgb(initial.color) },
                uSecondaryColor: { value: hexToRgb(initial.secondaryColor) },
                uTrailWidth: { value: initial.trailWidth },
                uTaper: { value: initial.trailTaper },
                uGlowIntensity: { value: initial.glowIntensity },
                uGlowSpread: { value: initial.glowSpread },
                uHotspot: { value: initial.hotspot },
                uBrightness: { value: initial.brightness },
                uOpacity: { value: initial.opacity },
                uPulseSpeed: { value: initial.pulseSpeed },
                uNoiseStrength: { value: initial.noiseStrength },
                uNormalBlend: { value: initial.blendMode === 'normal' ? 1 : 0 },
                uTime: { value: 0 },
                uFade: { value: 0 },
            },
            transparent: true,
    ...
    return (
        <div ref={containerRef} className="nocturne-glowcursor" aria-hidden="true">
            <canvas ref={canvasRef} style={{ mixBlendMode: blendMode }} />
        </div>
    );
};
